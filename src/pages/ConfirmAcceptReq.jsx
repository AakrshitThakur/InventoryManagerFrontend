...
export default function ConfirmAcceptReq() {
  ...
  const HandleAcceptReq = async () => {
    try {
      const response = await axios.get(
        `https://inventorymanagerbackend.onrender.com/reqs/${id}/accept`,
        {},
      ...
      if (response.data.AuthenticationError) {
        ...
        navigate("/reqs/ViewReqsReceived", { state: MsgObj });
      }
    } catch (FrontendError) {
  ...
  return (
    <div className="ConfirmDeleteShop relative bg-[url('/images/GeneralBgImg.png')] bg-cover bg-center flex-1 flex flex-col justify-center items-center p-1 md:p-2">
      ...
      >
        <h1 className="text-2xl md:text-3xl lg:text-4xl mb-1">
          Sure you want to accept this request?
        </h1>
        <div>
          <span className="mr-1">
            <button
          ...
          <span>
            <button className="text-xs md:text-sm lg:text-md px-1 py-1 md:px-2 md:py-2 bg-red-400 text-black rounded">
              <a href="/reqs/ViewReqsReceived">No</a>
            </button>
          </span>
        </div>
      </div>
